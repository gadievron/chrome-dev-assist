...
describe('Dogfooding - Extension Testing Itself', () => {
  ...
  describe('Basic Test Fixture', () => {
    ...
    test('opens basic test page and detects test identification', async () => {
      let tabId;
      try {
        ...
        const testIdLog = result.consoleLogs.find(log =>
          log.message && log.message.includes('Test ID:') && log.message.includes('basic-test-001')
        );
        ...
        const testNameLog = result.consoleLogs.find(log =>
          log.message && log.message.includes('Test Name:') && log.message.includes('Basic Test Page')
        );
        ...
        const extensionLog = result.consoleLogs.find(log =>
          log.message && log.message.includes('Extension:') && log.message.includes('Chrome Dev Assist')
        );
        ...
        const headerLog = result.consoleLogs.find(log =>
  ...
  describe('Console Errors Test Fixture', () => {
    ...
    test('captures expected errors from test page', async () => {
      let tabId;
      try {
        ...
        const result = await chromeDevAssist.openUrl(
          getFixtureUrl('console-errors-test.html'),
          {
            active: false,
            captureConsole: true,
        ...
        const testIdLog = result.consoleLogs.find(log =>
          log.message && log.message.includes('Test ID:') && log.message.includes('console-errors-001')
        );
        ...
        const hasError1 = errorMessages.includes('Error 1') || errorMessages.includes('ReferenceError');
        const hasError2 = errorMessages.includes('Error 2') || errorMessages.includes('TypeError');
        const hasError3 = errorMessages.includes('Error 3') || errorMessages.includes('test error');
  ...
  describe('Mixed Console Output Test Fixture', () => {
    ...
    test('captures mixed console output (logs, warnings, errors)', async () => {
      let tabId;
      try {
        ...
        const result = await chromeDevAssist.openUrl(
          getFixtureUrl('console-mixed-test.html'),
          {
            active: false,
            captureConsole: true,
            duration: 3000,
        ...
        // Validate test identification
        const testIdLog = result.consoleLogs.find(log =>
          log.message && log.message.includes('Test ID:') && log.message.includes('console-mixed-001')
        );
        ...
        // Count message types
        const logs = result.consoleLogs.filter(log => log.level === 'log' || log.level === 'info');
        const warnings = result.consoleLogs.filter(log => log.level === 'warn' || log.level === 'warning');
        const errors = result.consoleLogs.filter(log => log.level === 'error');

        console.log(`✓ Breakdown: ${logs.length} logs, ${warnings.length} warnings, ${errors.length} errors`);
  ...
  describe('Complete Test Workflow', () => {
    ...
    test('opens → reloads → captures → closes test page', async () => {
      let tabId;
      try {
        ...
        const opened = await chromeDevAssist.openUrl(
          getFixtureUrl('basic-test.html'),
          { active: false }
        );
        ...
        const testIdLog = reloaded.consoleLogs.find(log =>
          log.message && log.message.includes('basic-test-001')
        );
  ...
  describe('Concurrent Fixture Testing', () => {
    ...
    test('opens multiple test fixtures simultaneously', async () => {
      const tabIds = [];
      try {
        ...
        expect(results[0].consoleLogs.some(log =>
          log.message && log.message.includes('basic-test-001')
        )).toBe(true);
        console.log('✓ Fixture 1: basic-test-001 validated');

        expect(results[1].consoleLogs.some(log =>
          log.message && log.message.includes('console-errors-001')
        )).toBe(true);
        console.log('✓ Fixture 2: console-errors-001 validated');

        expect(results[2].consoleLogs.some(log =>
          log.message && log.message.includes('console-mixed-001')
        )).toBe(true);
        ...
        await Promise.all(tabIds.map(tabId =>
          chromeDevAssist.closeTab(tabId).catch(() => {})
        ));
