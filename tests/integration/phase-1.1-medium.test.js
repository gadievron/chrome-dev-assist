...
describe('Phase 1.1 - Medium Complexity Tests', () => {
  ...
  describe('Beginner Developer - Realistic Confusion', () => {
    ...
    test('passes wrong types to functions that expect none', async () => {
    ...
    test('forgets to await async functions', async () => {
    ...
    test('confuses tabId with URL string', async () => {
    ...
    test('uses negative or zero tabId', async () => {
  ...
  describe('Busy Developer - Rapid Workflows', () => {
    ...
    test('opens multiple tabs quickly', async () => {
      const tabs = [];
      try {
    ...
    test('open -> reload -> close workflow', async () => {
      let tabId;
      try {
    ...
    test('queries extension info multiple times', async () => {
  ...
  describe('Security Expert - Basic Injection Attempts', () => {
    ...
    test('handles SQL-like injection in extensionId', async () => {
      try {
        ...
        expect(err.message).toMatch(/extensionId must be 32 characters|Invalid extensionId format/i);
      }
    }, TEST_TIMEOUT);

    test('validates extensionId length strictly', async () => {
      // Too short
      try {
    ...
    test('validates extensionId character set', async () => {
  ...
  describe('Debugger - Error Details', () => {
    ...
    test('provides clear error for non-existent extension', async () => {
    ...
    test('returns detailed extension info including permissions', async () => {
      const { extensions } = await chromeDevAssist.getAllExtensions();
      expect(extensions.length).toBeGreaterThan(0);
    ...
    test('openUrl returns tabId for tracking', async () => {
      let tabId;
      try {
  ...
  describe('Reliability Engineer - Concurrent Operations', () => {
    ...
    test('handles parallel getAllExtensions calls', async () => {
    ...
    test('handles parallel getExtensionInfo calls', async () => {
      const { extensions } = await chromeDevAssist.getAllExtensions();
      expect(extensions.length).toBeGreaterThan(0);
    ...
    test('opens multiple tabs concurrently', async () => {
      const tabIds = [];
      try {
        // Open 3 tabs at once
        const promises = [
  ...
  describe('Perfectionist - Consistency and Edge Cases', () => {
    ...
    test('getAllExtensions count matches array length', async () => {
      const result = await chromeDevAssist.getAllExtensions();
      expect(result.count).toBe(result.extensions.length);
    }, TEST_TIMEOUT);

    test('getAllExtensions filters out Chrome Dev Assist itself', async () => {
      const result = await chromeDevAssist.getAllExtensions();

      // Should not include self
      const selfFound = result.extensions.some(ext =>
        ext.name === 'Chrome Dev Assist'
      );
      expect(selfFound).toBe(false);
    }, TEST_TIMEOUT);

    test('openUrl with active:true focuses tab', async () => {
      let tabId;
      try {
    ...
    test('openUrl with active:false backgrounds tab', async () => {
      let tabId;
      try {
        const result = await chromeDevAssist.openUrl('https://example.com', {
          active: false
    ...
    test('reloadTab with bypassCache option', async () => {
      let tabId;
      try {
        // Open tab first
        const opened = await chromeDevAssist.openUrl('https://example.com', { active: false });
    ...
    test('closeTab returns correct response', async () => {
      // Open tab
      const opened = await chromeDevAssist.openUrl('https://example.com', { active: false });
