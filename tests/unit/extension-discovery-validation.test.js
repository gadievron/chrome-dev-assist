...
describe('Extension Discovery - Security Validation', () => {
  describe('validateExtensionId()', () => {
    ...
    test('should reject extension ID with letters q-z (outside base-32 alphabet)', () => {
      ...
      expect(() => validateExtensionId(invalidId)).toThrow(
    ...
    test('should reject null extension ID', () => {
      expect(() => validateExtensionId(null)).toThrow('extensionId must be non-empty string');
    });

    test('should reject undefined extension ID', () => {
      expect(() => validateExtensionId(undefined)).toThrow('extensionId must be non-empty string');
    });

    test('should reject empty string extension ID', () => {
      expect(() => validateExtensionId('')).toThrow('extensionId must be non-empty string');
    });

    test('should reject number extension ID', () => {
      expect(() => validateExtensionId(123)).toThrow('extensionId must be non-empty string');
    });

    // Security cases
    test('should reject SQL injection attempt', () => {
      const maliciousId = "'; DROP TABLE extensions; --";
      expect(() => validateExtensionId(maliciousId)).toThrow('Invalid extension ID format');
    });
  ...
  describe('validateMetadata()', () => {
    ...
    test('should reject metadata larger than 10KB', () => {
      ...
      expect(() => validateMetadata(metadata)).toThrow('Metadata too large (max 10KB)');
    });
  ...
  describe('validateCapabilities()', () => {
    ...
    test('should reject non-array capabilities', () => {
      const capabilities = 'test-orchestration';
      expect(() => validateCapabilities(capabilities)).toThrow('capabilities must be array');
    });

    test('should reject null capabilities', () => {
      expect(() => validateCapabilities(null)).toThrow('capabilities must be array');
    });

    test('should reject capabilities with non-string values', () => {
      const capabilities = [123, 'test-orchestration'];
      expect(() => validateCapabilities(capabilities)).toThrow('capability must be string');
    });

    // Invalid cases - unknown capability
    test('should reject unknown capability', () => {
      const capabilities = ['unknown-capability'];
      expect(() => validateCapabilities(capabilities)).toThrow(
    ...
    test('should reject typo in capability name', () => {
      ...
      expect(() => validateCapabilities(capabilities)).toThrow(
        'Unknown capability: test-orchestraton'
      );
    });
  ...
  describe('validateName()', () => {
    ...
    test('should reject null name', () => {
      expect(() => validateName(null)).toThrow('name must be non-empty string');
    });

    test('should reject empty name', () => {
      expect(() => validateName('')).toThrow('name must be non-empty string');
    });

    test('should reject number name', () => {
      expect(() => validateName(123)).toThrow('name must be non-empty string');
    });

    // Invalid cases - length
    test('should reject name longer than 100 chars', () => {
      const name = 'x'.repeat(101);
      expect(() => validateName(name)).toThrow('name too long (max 100 characters)');
    });

    // Invalid cases - XSS prevention
    test('should reject name with < character (XSS)', () => {
      expect(() => validateName('Extension <script>')).toThrow('name contains invalid characters');
    });

    test('should reject name with > character (XSS)', () => {
      expect(() => validateName('Extension >')).toThrow('name contains invalid characters');
    });

    test('should reject name with & character', () => {
      expect(() => validateName('Extension & Co')).toThrow('name contains invalid characters');
    });

    test('should reject name with " character', () => {
      expect(() => validateName('Extension "quoted"')).toThrow('name contains invalid characters');
    });
  ...
  describe('validateVersion()', () => {
    ...
    test('should reject null version', () => {
      expect(() => validateVersion(null)).toThrow('version must be non-empty string');
    });

    test('should reject empty version', () => {
      expect(() => validateVersion('')).toThrow('version must be non-empty string');
    });

    test('should reject number version', () => {
      expect(() => validateVersion(1.0)).toThrow('version must be non-empty string');
    });

    // Invalid cases - format
    test('should reject version without patch number', () => {
      expect(() => validateVersion('1.0')).toThrow('version must be semantic version');
    });

    test('should reject version with only major number', () => {
      expect(() => validateVersion('1')).toThrow('version must be semantic version');
    });

    test('should reject version with text', () => {
      expect(() => validateVersion('v1.0.0')).toThrow('version must be semantic version');
    });

    test('should reject version with pre-release tag', () => {
      expect(() => validateVersion('1.0.0-beta')).toThrow('version must be semantic version');
    });

    test('should reject version with build metadata', () => {
      expect(() => validateVersion('1.0.0+build')).toThrow('version must be semantic version');
    });
  });
