...
describe('captureScreenshot() - Input Validation', () => {
  describe('Tab ID Validation', () => {
    test('should reject non-number tab ID (string)', async () => {
      await expect(captureScreenshot('not-a-number')).rejects.toThrow(/tab id must be a number/i);
    });

    test('should reject non-number tab ID (null)', async () => {
      await expect(captureScreenshot(null)).rejects.toThrow(/tabid is required/i);
    });

    test('should reject non-number tab ID (undefined)', async () => {
      await expect(captureScreenshot(undefined)).rejects.toThrow(/tabid is required/i);
    });

    test('should reject non-number tab ID (object)', async () => {
      await expect(captureScreenshot({})).rejects.toThrow(/tab id must be a number/i);
    });

    test('should reject negative tab ID', async () => {
      await expect(captureScreenshot(-1)).rejects.toThrow(/tab id must be a positive number/i);
    });

    test('should reject zero tab ID', async () => {
      await expect(captureScreenshot(0)).rejects.toThrow(/tab id must be a positive number/i);
    });
  });
  ...
  describe('Tab ID Edge Cases (Previously Broken - Fixed in P0)', () => {
    ...
    test('should reject very large float (MAX_SAFE_INTEGER + 0.5)', async () => {
      await expect(captureScreenshot(Number.MAX_SAFE_INTEGER + 0.5)).rejects.toThrow(
        /tab id exceeds safe integer range/i
      );
    });
  });
  ...
  describe('Format Validation', () => {
    test('should reject invalid format (gif)', async () => {
      ...
      await expect(captureScreenshot(12345, { format: 'gif' })).rejects.toThrow(
        /format must be "png" or "jpeg"/i
      );
    });

    test('should reject invalid format (bmp)', async () => {
      await expect(captureScreenshot(12345, { format: 'bmp' })).rejects.toThrow(
        /format must be "png" or "jpeg"/i
      );
    });

    test('should reject invalid format (webp)', async () => {
      await expect(captureScreenshot(12345, { format: 'webp' })).rejects.toThrow(
        /format must be "png" or "jpeg"/i
      );
    });
  });
  ...
  describe('Quality Validation (JPEG)', () => {
    test('should reject quality < 0', async () => {
      await expect(captureScreenshot(12345, { format: 'jpeg', quality: -10 })).rejects.toThrow(
        /quality must be between 0 and 100/i
      );
    });

    test('should reject quality > 100', async () => {
      await expect(captureScreenshot(12345, { format: 'jpeg', quality: 150 })).rejects.toThrow(
        /quality must be between 0 and 100/i
      );
    });

    test('should reject quality = -1', async () => {
      await expect(captureScreenshot(12345, { format: 'jpeg', quality: -1 })).rejects.toThrow(
        /quality must be between 0 and 100/i
      );
    });

    test('should reject quality = 101', async () => {
      await expect(captureScreenshot(12345, { format: 'jpeg', quality: 101 })).rejects.toThrow(
        /quality must be between 0 and 100/i
      );
    });

    /**
     * NEW TEST: P2-2 Integer Validation
     * Quality must be an integer (no fractional values)
     */
    test('should reject fractional quality (75.5)', async () => {
      await expect(captureScreenshot(12345, { format: 'jpeg', quality: 75.5 })).rejects.toThrow(
        /quality must be an integer/i
      );
    });

    test('should reject fractional quality (0.5)', async () => {
      await expect(captureScreenshot(12345, { format: 'jpeg', quality: 0.5 })).rejects.toThrow(
        /quality must be an integer/i
      );
    });

    test('should reject fractional quality (99.9)', async () => {
      await expect(captureScreenshot(12345, { format: 'jpeg', quality: 99.9 })).rejects.toThrow(
        /quality must be an integer/i
      );
    });
  });
  ...
  describe('Valid Inputs (will fail at connection, not validation)', () => {
    ...
    test('should accept valid tab ID with JPEG format and quality', async () => {
      await expect(captureScreenshot(12345, { format: 'jpeg', quality: 80 })).rejects.toThrow(
        /Extension not connected|ECONNREFUSED/i
      );
    });
